...
void System::drawDebugString(Light* light, Player* player)
{
    ...
    DrawFormatString(0, 120, cr, "ライト座標A:%f,%f", light->posA.x, light->posA.y);
    DrawFormatString(0, 140, cr, "ライト座標B:%f,%f", light->posB.x, light->posB.y);
    DrawFormatString(0, 160, cr, "ライト座標C:%f,%f", light->posC.x, light->posC.y);
    DrawFormatString(0, 180, cr, "ライト座標D:%f,%f", light->posD.x, light->posD.y);
    DrawFormatString(0, 200, cr, "ライト座標:%f,%f", light->pos.x, light->pos.y);
    DrawFormatString(0, 220, cr, "after座標A:%f,%f", light->after_posA.x, light->after_posA.y);
    DrawFormatString(0, 240, cr, "after座標B:%f,%f", light->after_posB.x, light->after_posB.y);
    DrawFormatString(0, 260, cr, "after座標C:%f,%f", light->after_posC.x, light->after_posC.y);
    DrawFormatString(0, 280, cr, "after座標D:%f,%f", light->after_posD.x, light->after_posD.y);
}
